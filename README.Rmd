...
bbk is minimal R client for the [Bundesbank](https://www.bundesbank.de/en/statistics/time-series-databases/help-for-sdmx-web-service) and the [ECB API](https://data.ecb.europa.eu/help/api/overview) APIs.
...
bbk functions are prefixed with either `bbk_` or `ecb_` depending on the origin of the data and follow the naming convention of the APIs.
...
and then use it to retrieve the data with either `bbk_data()` or `bkb_series()` depending on your needs.
Despite querying different endpoints, both functions should return the same data.
However, `bbk_data()` is more flexible and allows more customisation.
