...
Retrieve the available indicators and their attributes from the CNB ARAD
database. Use this to discover indicator identifiers for
...
with one row per indicator and columns including `indicator_id`,
`indicator_name`, `frequency_code`, `frequency_name`, `unit_mult_code`,
