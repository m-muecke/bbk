...
Retrieve the dimension structure of indicators from the CNB ARAD
database, returning one row per indicator dimension.
...
with columns including `indicator_id`, `base_code`, `base_name`,
`dim_code`, `dim_name`, `dim_value_code`, `dim_value_name`, and
`dim_rank`.
